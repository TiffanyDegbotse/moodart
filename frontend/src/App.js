...
function ResultPanel({ original, result, style, method }) {
  ...
  const stylizedSrc = isNeural
    ? NEURAL_GALLERY[selectedGalleryStyle]
      ...
      {isNeural && (
        <div className="neural-gallery">
          ...
          <div className="neural-gallery-grid">
            {Object.entries(NEURAL_GALLERY).map(([name, src]) => (
              <div
                key={name}
...
export default function App() {
  ...
  const handleStylize = useCallback(async () => {
    ...
    // Neural method uses pre-rendered gallery — no API call needed
                ...
                >
                  ...
                  {selectedMethod === 'neural' && (
                    <motion.div className="neural-note" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ✦ Neural mode shows pre-rendered results from our Magenta NST model — click any style to explore
                    </motion.div>
                  )}
